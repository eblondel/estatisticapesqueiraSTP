...
append_and_update <- function(
      ...
      function(datname)
        if(is.null(all_data_local[[datname]]) &
           !is.null(all_data[[datname]])) TRUE else
             length(
               which(
                 !all_data[[datname]]$uuid %in%
                   all_data_local[[datname]]$uuid)
             ) > 0
    ))
  ]
  ...
  ### ----------- 4. GET DATA WITH NEW SUBMISSIONS  ----------- ###
  ...
  if(length(newdat) == 0  &
     !update_server_data &
     is_conf_same) {
    message(msg$there_are_no_new_submissions)
    return(all_data)
  } else {
    ...
    if(update_server_data | !is_conf_same){
    ...
    for(i in 1:length(all_data)){
      ...
      rem = local_data_filepaths$filepath[local_data_filepaths$dat == datname]
      if(length(rem) > 0) remove_files(rem, language = language)
      ...
      filepaths <- write_newdata(
        ...
        local_data_directory = if(is_raw_data) file.path(tempdir(), now) else local_data_directory,
  ...
  remove_files(file.path(cache, "settings.rds"), language = language)
