...
write_newdata <- function(data,
  ...
  if(method == "FULL"){
    message(paste0(msg$File, " '", name, ".", fileformat, "' ", msg$saved_in, " '", local_data_directory, "'"))
    writeDAT(data = data,
             fileformat = fileformat,
  ...
  if(method == "GROUPED"){
    ...
    for(directory in out_dirs){
      ...
      message(paste0(msg$File, " '", filename, "' ", msg$saved_in, " '", path, "'"))

      writeDAT(data       = data[which(grouping_vector == directory), ],
               fileformat = fileformat,
